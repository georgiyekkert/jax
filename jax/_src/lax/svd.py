...
def _constant_svd(
    a: Any, return_nan: bool, full_matrices: bool, compute_uv: bool = True, subset_by_index: tuple[int, int] | None = None,
) -> Any | Sequence[Any]:
...
def _svd_tall_and_square_input(
    a: Any, hermitian: bool, compute_uv: bool, max_iterations: int, subset_by_index: tuple[int, int] | None = None,
...
def _qdwh_svd(a: Any,
              ...
              subset_by_index: tuple[int, int] | None = None) -> Any | Sequence[Any]:
...
def svd(a: Any,
  ...
  if subset_by_index is not None:
    ...
    subset_by_index = core.concrete_or_error(
      int, subset_by_index, 'The `subset_by_index` argument must be statically '
      'specified to use `svd` within JAX transformations.')
    if full_matrices:
      raise ValueError('full_matrices and subset_by_index cannot be both be set.')
