...
#define NYI(msg)                            \
  ...
  LogicalResult infer(vector::BroadcastOp op) {
    ...
    if (auto src_ty = dyn_cast<VectorType>(some_src_ty)) {
      ...
      setLayout(
          op,
          VectorLayout(some_layout->bitwidth(), some_layout->offsets(),
                       nativeTiling(some_layout->bitwidth()),
                       some_layout->implicit_dim()),
          VectorLayout(layout.bitwidth(), offsets,
                       nativeTiling(layout.bitwidth()), ImplicitDim::kNone));
      return success();
    }
