...
LogicalResult tpu_rotate_rule(RewriteContext &ctx, Operation &op,
  ...
  if (vty.getRank() <= rotate_op.getDimension() ||
      rotate_op.getDimension() < 0) {
    return rotate_op.emitOpError("Invalid dimension: ")
           << rotate_op.getDimension();
  }
  if (rotate_op.getStrideDimension().has_value() &&
      (vty.getRank() <= rotate_op.getStrideDimension().value() ||
       rotate_op.getStrideDimension().value() < 0)) {
    return rotate_op.emitOpError("Invalid stride dimension: ")
           << rotate_op.getStrideDimension().value();
  }
  if (rotate_op.getStride().has_value() !=
      rotate_op.getStrideDimension().has_value()) {
    return rotate_op.emitOpError(
        "Expected both stride and stride dimension are specified or not "
        "specified");
  }
    ...
    if (axis >= vregs.num_dimensions() - 2) {
      ...
      for (int64_t i = 0; i < chunks.size(); ++i) {
        chunks[i].Each([&](absl::Span<const int64_t> idxs, Value *v) {
          *v = stride > 0
                   ? builder.create<tpu::RotateOp>(
                         res_vreg_ty, *v, shift_in_vreg,
                         /*dimension=*/tiling_dim,
                         builder.getI32IntegerAttr(stride),
                         /*stride_dimension=*/builder.getI32IntegerAttr(0))
                   : builder.create<tpu::RotateOp>(
                         res_vreg_ty, *v, shift_in_vreg,
                         /*dimension=*/tiling_dim,
      ...
      // After rotation on each verg, we need to select the wrapped data
  ...
  if (rotate_op.getStride().has_value() &&
    ...
    if (stride_dim == dim) {
      // No rotation needed if stride dimension and rotate dimension are equal.
      out_tiles = in_tiles;
    } else if (stride_dim == vty.getRank() - 1) {
      return op.emitOpError(
